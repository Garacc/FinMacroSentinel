...
4. **强制溯源机制**：所有客观事实必须附带原始新闻的有效URL超链接，格式为 \`[信源名称](URL)\`。严禁捏造虚假链接（防幻觉）。
...
### Ⅱ. 逻辑传导与资产映射
* **传导链路 (Mechanism)**：[强制填写因果链条，例：通胀超预期 -> 降息预期延后 -> 实际利率上行 -> 压制无现金流资产估值]

* **时点与定性 (Timeframe)**：[短期流动性冲击 / 中期基本面趋势 / 长期宏观叙事] —— 预计逻辑演绎周期：[X周/月]

* **资产配置指令 (Asset Mapping)**：
    * 🟢 **多头/增配 (Long)**：[例：美元指数、短期美债ETF、价值红利型宽基]
    * 🔴 **空头/规避 (Short/Avoid)**：[例：长久期美债、无盈利生物科技ETF]

### Ⅲ. 风险控制与边界条件 (核心风控)
* **逻辑证伪点 (Falsification)**：[必须明确：什么数据/事件出现，说明上述推演是错的？例：若周五非农新增就业跌破10万，则市场将瞬间切换为"衰退交易"，上述逻辑作废。]
...
### Ⅱ. 逻辑传导与资产映射
* **传导链路 (Mechanism)**：[阐述产业链上下游的利润分配或产能传导机制]

* **时点与定性 (Timeframe)**：[产业爆发期 / 渗透率瓶颈 / 产能出清期]

* **资产配置指令 (Asset Mapping)**：
    * 🟢 **多头/增配 (Long)**：[具有议价权的环节或对应ETF]
    * 🔴 **空头/规避 (Short/Avoid)**：[利润受挤压的环节或被过度炒作的板块]

### Ⅲ. 风险控制与边界条件 (核心风控)
* **逻辑证伪点 (Falsification)**：[指出该产业逻辑破灭的先兆指标]
